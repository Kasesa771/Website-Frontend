      ...
      <UTable v-model="selected" :rows="filteredItems" :columns="columns" :loading="loading"
        ...
        <template #profile-data="{ row }">
          <div class="border rounded-lg h-14 w-14 flex items-center justify-center">
            <img :src="row.profile" alt="Users Image" class="w-12 h-12 object-cover rounded-lg" />
          </div>
        </template>
...
import { useUsersStore } from '~/stores/dashboard/users.store';
...
const viewUsers = (item: any) => {
  ...
  store.setSelectedUser(item)
}

const deleteUsers = (item: any) => {
